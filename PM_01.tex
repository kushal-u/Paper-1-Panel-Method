...
Improving airfoil performance without active actuation remains an important objective in aerospace and energy systems. Conventional high-lift and flow-control devices can be effective, but they may introduce mechanical complexity, additional mass, maintenance requirements, and external geometric discontinuities. Passive flow-control concepts offer a complementary approach in which the pressure field surrounding an aerodynamic body supplies the energy required to modify the flow. Porous surfaces, permeable regions, discrete apertures, and internal passages all exploit this principle by permitting fluid transport between locations of unequal pressure. Their effectiveness, however, depends on where the openings are placed, how they are connected, and how strongly the resulting transpiration interacts with the external aerodynamic field.

Early investigations established that surface porosity can substantially alter the pressure loading of an airfoil, but also demonstrated that porosity is not inherently beneficial. Mineck and Hartwich examined full-chord porosity on a NACA~0012 airfoil in transonic flow and found that the porous surface generally reduced the lift-curve slope and increased drag at a specified normal-force coefficient \cite{Mineck1996FullChordPorosity}. Experiments by Aldheeb et al. similarly showed that increasing porosity reduced the lift-curve slope, while the drag response was non-monotonic, initially decreasing at low porosity before increasing as porosity became larger \cite{Aldheeb2018PorousAirfoilsWings}. These results illustrate that unrestricted pressure equalisation can weaken the pressure difference responsible for lift and that porosity does not produce a universally favourable aerodynamic response. Useful passive flow control therefore requires selective permeability or controlled connectivity rather than indiscriminate porosity across the airfoil.

Reduced-order theories have provided important insight into the aerodynamic influence of distributed permeability. Hajian and Jaworski introduced a steady thin-airfoil formulation in which a local Darcy condition couples the pressure jump across the airfoil to seepage through a chordwise porosity distribution \cite{Hajian2017PorosityGradients}. Luesutthiviboon et al. assessed a related thin-airfoil treatment for partially porous airfoils and identified reduced accuracy when the porous extent is small or the flow resistivity is low \cite{Luesutthiviboon2019ThinAirfoilAssessment}. Baddoo et al. subsequently extended porous-airfoil theory to unsteady motion and gust response by incorporating resistance and effective-fluid-inertia terms into the porous boundary condition \cite{Baddoo2021UnsteadyPorousAerofoils}. These models are computationally efficient and clarify the relationship between pressure difference, resistance, and aerodynamic loading. Nevertheless, thin-airfoil formulations neglect the finite thickness of the airfoil, and their porous conditions primarily describe local pressure--suction-side communication through a distributed or homogenised surface rather than arbitrary finite-length passages connecting selected, non-local surface locations.
...
An alternative class of passive devices uses internal slots or bleed passages to transport fluid from a high-pressure region to a lower-pressure region. Acarer optimised passive slots for the DU12W262 airfoil and reported an improvement in peak lift-to-drag ratio, illustrating the potential of pressure-driven internal routing for aerodynamic enhancement \cite{Acarer2020DU12W262PassiveFlow}. Kestel et al. used passive bleeding through internal passages to alter the separated vortex structure over a non-slender delta wing, directly demonstrating flow transport driven by the natural pressure difference between connected surfaces \cite{Kestel2020PassiveBleeding}. More recently, Bahadi et al. parametrically investigated a curved passive internal slot in a NACA~4412 airfoil and reported improved high-incidence performance for selected inlet, taper, and curvature combinations \cite{Bahadi2026CurvedPassiveSlot}. These studies establish internal pressure-driven passages as prior art and therefore preclude broad claims of introducing passive surface connectivity. Most such investigations, however, consider a particular slot geometry using experiments or Reynolds-averaged Navier--Stokes calculations rather than a general reduced-order representation of multiple independently connected opening pairs.

Porous-airfoil design is increasingly being treated as an optimisation problem because the aerodynamic response depends on several interacting geometric and material variables. Zamani et al. applied a parameter-screening and optimisation procedure to a porous NACA~0015 airfoil for static-stall control, varying the porous-region dimensions, location, porosity, and cell size \cite{Zamani2026NACA0015PorousStall}. This work demonstrates the value of systematic design exploration, but the associated porous layer is described through distributed material parameters. For airfoils containing discrete passages, the design variables also include the identity and location of the connected surface openings, passage length, orientation, aperture, and the combination of same-surface and cross-surface routes. Exploring this larger topological design space exclusively with viscous computational fluid dynamics or experiments would be computationally expensive at the preliminary-design stage.

Panel methods provide an intermediate modelling level between analytical thin-airfoil theory and viscous computational fluid dynamics. The foundational work of Hess and Smith established the boundary-element representation of potential flow by distributing singularities over a body surface, enabling the pressure field around geometrically resolved bodies to be obtained without discretising the surrounding fluid domain \cite{HessSmith1967PotentialFlow}. The subsequent review by Hess documented the development, capabilities, and limitations of panel methods as efficient aerodynamic boundary-integral techniques \cite{Hess1990PanelMethods}, while Birk provided a modern and implementation-oriented treatment of the constant-source and dipole Hess--Smith formulation \cite{Birk2022HessSmithGuide}. For a two-dimensional lifting airfoil, a source distribution can enforce the surface-normal boundary condition, while circulation and a trailing-edge Kutta condition provide the lifting solution. This structure is particularly suitable for the present problem because the impermeability condition can be replaced locally by a prescribed normal transpiration velocity at each passage opening. The resulting surface velocity gives the pressure coefficient directly, providing the endpoint pressure difference that drives the internal hydraulic calculation. Panel methods are also sufficiently inexpensive for repeated solution across many passage layouts, sizes, and angles of attack.

Established viscous--inviscid methods further demonstrate how potential-flow panel solutions can form the aerodynamic core of more complete airfoil-analysis frameworks. Drela and Giles developed a strongly coupled viscous--inviscid method for transonic and low-Reynolds-number airfoils, combining an inviscid outer-flow solution with integral boundary-layer equations \cite{DrelaGiles1987ViscousInviscid}. XFOIL subsequently provided a rapid airfoil-analysis and design system based on an inviscid linear-vorticity panel method coupled to integral boundary-layer and transition models \cite{Drela1989XFOIL}. These methods can predict viscous effects that are outside the present formulation, but their panel-based outer solutions support the use of potential flow for efficient surface-pressure calculation. In the present study, XFOIL is used only as a reference for the impermeable baseline; the proposed external-flow solver intentionally remains inviscid so that the pressure-driven aero--hydraulic coupling can be isolated and evaluated at low computational cost. This separation also makes the model suitable for preliminary screening, after which selected passage configurations can be examined using viscous computational fluid dynamics or experiments.

The reviewed literature therefore spans distributed Darcy-type surfaces, homogenised porous media, wall-normal holes, individual passive slots, and established potential-flow panel formulations. However, a modelling gap remains between local permeability boundary conditions and high-fidelity simulations of fixed internal geometries. In particular, limited attention has been given to a panel-based reduced-order framework that explicitly maps selected pairs of surface openings to finite-length passages, determines the pressure-driven flow through each passage, and feeds the resulting suction and blowing velocities back into the external aerodynamic solution. The systematic comparison of non-local same-surface passages, pressure-to-suction-surface passages, and their combined but hydraulically independent arrangement also remains underexplored. Addressing this gap would provide a computationally efficient basis for screening large numbers of candidate passage networks before higher-fidelity viscous simulation and experimental validation.

Accordingly, the present study develops a two-dimensional coupled aero--hydraulic framework for a NACA~0018 airfoil. The external flow is represented by a steady, incompressible, inviscid, and irrotational source--vortex panel method with a Kutta condition. Selected surface openings are connected by passive rectangular passages, and the pressure difference between the paired endpoints determines the internal flow through a hydraulic-resistance relation. The corresponding opening-normal velocities are iteratively coupled to the external panel solution. Three connectivity configurations are examined: four chordwise same-surface passages, four pressure-to-suction-surface passages, and a combined arrangement containing all eight passages as hydraulically independent connections. Passage heights of 8, 9, and 10~mm are assessed over angles of attack from $-5^{\circ}$ to $15^{\circ}$ at a chord Reynolds number of $5\times10^{5}$, with comparisons against the impermeable airfoil and an XFOIL reference solution.

The principal contribution is the explicit reduced-order representation of prescribed, finite-length, pairwise surface connectivity within a coupled panel--hydraulic solver. The formulation permits different passage topologies to be evaluated within one consistent aerodynamic model and exposes the relationship between endpoint pressure difference, internal flow rate, surface transpiration, pressure distribution, lift, pitching moment, and pressure-based streamwise force. The method is intended for rapid preliminary assessment and topology screening. Because the external-flow model does not resolve boundary layers, skin-friction drag, separation, or stall, the results are interpreted as inviscid aerodynamic trends and a basis for selecting configurations for subsequent viscous computational and experimental validation.
...
\bibliography{porous_airfoil_literature}
\end{document}
